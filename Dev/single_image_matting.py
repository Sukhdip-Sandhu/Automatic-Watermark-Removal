...
def single_image_matting(images, W):
    debug(print, "Starting single image matting...")
    W = (W * 255).astype(np.uint8)
    _, scribbles = cv2.threshold(W, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    scribbles = (scribbles / 255).astype(np.float64)
    scribbles_3d = np.stack((scribbles, scribbles, scribbles), axis=-1)
    si_mattes = np.array([
            closed_form_matting_with_scribbles(np.stack((images[k], images[k], images[k]), axis=-1), scribbles_3d)
                for k in range(images.shape[0])
        ])
    return np.median(si_mattes, axis=0)
